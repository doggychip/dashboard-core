...
// YYYY-MM-DD stamp for data.refreshedAt.
function todayStamp() {
...
async function updateTickers(data) {
  ...
  for (const tk of tickers) {
    if (data.tickers[tk] && (data.tickers[tk].status === 'acquired' || data.tickers[tk].status === 'delisted')) {
      console.log(`  - ${tk.padEnd(8)} skipped (${data.tickers[tk].status})`);
      continue;
    }
    try {
  ...
  if (ok > 0) {
    // Freshness stamp — client-side signals gate the 20d-momentum component
    // on this, so a stale priceHistory can't masquerade as a 20-session window.
    data.refreshedAt = todayStamp();
  }
...
module.exports = { runSoftwareSchemaInline, runSoftwareSchemaExternal, safeStringify, updateTickers, todayStamp };
